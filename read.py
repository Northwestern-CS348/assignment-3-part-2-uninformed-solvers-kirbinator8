...
def getForbiddensFactStrings(filenameOrAbsoluteFilePath):
    ...
    if '\\' in filenameOrAbsoluteFilePath or '/' in filenameOrAbsoluteFilePath:
        relativePath = filenameOrAbsoluteFilePath  # absolute path
    else:
        relativePath = os.path.join('flatfiles', filenameOrAbsoluteFilePath)
    with open(relativePath) as forbidden_facts:
        for line in forbidden_facts:
            s = line.strip()
